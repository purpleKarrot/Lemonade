...
class ArchiveDriver: Driver, Object
  {
  public ArchiveDriver(string machine, string sysname)
    {
  ...
  private void filter(Karrot.Dictionary fields, Karrot.AddFun add_fun)
    {
    unowned string p_sysname = fields.lookup("sysname");
    if (p_sysname != "*" && p_sysname != sysname)
      {
      return;
      }
    unowned string p_machine = fields.lookup("machine");
    if (p_machine != "*" && p_machine != machine)
      {
      return;
      }
    unowned string href = fields.lookup("href");
    unowned string checksum = fields.lookup("checksum");
    add_fun({"href", href, "checksum", checksum}, false);
    }
  private void download(Karrot.Implementation impl, bool requested) throws Error
    {
    unowned Karrot.Dictionary vals = impl.values;
    string file = downloader.get(vals.lookup("href"), vals.lookup("checksum"));

    unowned string dest = impl.name;
    string temp = dest + "-tmp";
    var tempdir = new TempDir(temp);

    extract_local(file);
    var wanted = analyze_extracted(temp);
    copy_folder(wanted, dest);
    }
  private static string analyze_extracted(string directory) throws Error
    {
    bool found = false;
  ...
  private static void copy_folder(string src, string dst) throws Error
    {
  ...
  private static Archive.Result copy_data(Archive.Read ar, Archive.Write aw)
    {
    while (true)
      {
      void* buff;
  ...
  private static int extract_local(string archive_path)
    {
    Archive.Result ret = Archive.Result.OK;
